...
bot.on("ready", async () => {
    ...
    await bot.editStatus("online", { type: 0, name: process.env["status"] || "#Shadow Ticket ." })
...
bot.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.type == 3) {
        ...
        if (interaction.data.custom_id.startsWith("ticket")) {
            ...
            await interaction.createMessage({
                flags: 64,
                content: "**قـيـد الإنـشـاء <a:Shadow29:1136547779716587520> **"
            })
            ...
            bot.createChannel(interaction.guildID, `ticket-${padNum(count + 1, 4)}`, 0, {
                ...
                await interaction.editOriginalMessage({
                    flags: 64,
                    content: `**<a:Shadow17:1136543581843230742> تـم إنـشـاء تـذكـرتـك <#${channel.id}> .**`,
                })
                await bot.createMessage(interaction.data.custom_id.split("_")[4], {
                    embeds: [{
